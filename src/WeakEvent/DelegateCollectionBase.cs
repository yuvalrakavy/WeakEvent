...
namespace WeakEvent {
    internal abstract class DelegateCollectionBase<TOpenEventHandler, TStrongHandler>
        ...
        private static readonly ConcurrentDictionary<MethodInfo, TOpenEventHandler> s_openHandlerCache =
            new ConcurrentDictionary<MethodInfo, TOpenEventHandler>();

        private static readonly Type s_eventArgsType = typeof(TOpenEventHandler)
            .GetRuntimeMethods()
        ...
        private static TOpenEventHandler CreateOpenHandler(MethodInfo method) {
            ...
            var e = Expression.Parameter(s_eventArgsType, "e");
        ...
        public void Add(object? lifetimeObject, Delegate[] invocationList) {
            foreach(var singleHandler in invocationList) {
                var openHandler = s_openHandlerCache.GetOrAdd(singleHandler.GetMethodInfo(), CreateOpenHandler);
        ...
        public void Remove(Delegate[] invocationList) {
            var matchIndex = GetIndexOfInvocationListLastOccurrence(invocationList);

            if(matchIndex < 0)
                return;

            for(var invocationIndex = invocationList.Length - 1; invocationIndex >= 0; invocationIndex--) {
                ...
                if(_index.TryGetValue(hashCode, out var indices)) {
                    var lastIndex = indices.LastIndexOf(index);
                    if(lastIndex >= 0) {
        ...
        public void CompactHandlerList() {
            ...
            var newCount = _delegates.Count - _deletedCount;
            ...
            for(var oldIndex = 0; oldIndex < _delegates.Count; oldIndex++) {
            ...
            foreach(var hashCode in hashCodes) {
                if(_index[hashCode].Count == 0) {
                    ...
                    foreach(var oi in oldIndexList) {
                        if(newIndices.TryGetValue(oi, out var ni)) {
        ...
        private int GetIndexOfInvocationListLastOccurrence(Delegate[] invocationList) {
            var lastMatchStartIndex = _delegates.Count;
            while(lastMatchStartIndex > 0) {
                var currentIndex = -1;
                for(var handlerIndex = invocationList.Length - 1; handlerIndex >= 0; handlerIndex--) {
